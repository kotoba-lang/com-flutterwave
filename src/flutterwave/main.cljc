(ns flutterwave.main
  "Kotodama WASM entrypoint for the Flutterwave clean-room actor (L4) — Clojure port.
  ...
  facts (`flutterwave.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "flutterwave")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"flutterwave.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "flutterwave-compat" :tier tier :entities entities} 200])
...
(defn start! [] :flutterwave-compat/ready)
